        ...
        <> 

            {
                products.map((product)=>{
                    return<Accordion style={{width:"270px"}}>
                        <Accordion.Item eventKey="0" className={styles.accordin}>
                           <Accordion.Header> <Link style={{textDecoration:"none"}}   to={`/product-catalog${product.url}/${product.id}`} ><span className="fw-bold">{product.name}</span></Link></Accordion.Header>
                            ...
                            <Accordion.Body>
                                ...
                                <Accordion.Item eventKey="1" className={styles.accordin} >
                                  <Accordion.Header style={{border:"none"}}> <Link className='Link' style={{textDecoration:"none"}} to={`/product-catalog/${subproduct.name}/${subproduct.id}`}><span className="fw-bold">{subproduct.name}</span></Link></Accordion.Header> 

                                    <Accordion.Body style={{border:"none"}}>
                                        {
                                            subproduct.subsubCategories ? subproduct.subsubCategories.map((subsubproduct)=>{
                                                return <Accordion.Body><Link  className='Link' to={`/product-catalog/products/${subsubproduct.name}/${subsubproduct.id}`} style={{textDecoration:"none",color:"black"}} >  <span style={{colorL:"black"}} className="fw-bold">{subsubproduct.name}</span> </Link></Accordion.Body>
                                                    {/* <Accordion.Item eventKey="2" className={styles.accordin}>
                                               <Accordion.Header> <Link  className='Link' to={`/product-catalog/${subsubproduct.name}/${subsubproduct.id}`} style={{textDecoration:"none",color:"black"}} >  <span style={{colorL:"black"}} className="fw-bold">{subsubproduct.name}</span> </Link> </Accordion.Header>  
                                                </Accordion.Item> */}
                                    ...
                                    return <Accordion.Body> <Link className='Link' style={{textDecoration:"none"}} to={`/product-catalog/products/${subproduct.name}/${subproduct.id}`}><span className="fw-bold">{subproduct.name}</span></Link></Accordion.Body>
                                }):null
                                }
                            </Accordion.Body>
